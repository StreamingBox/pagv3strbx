module.exports = {
    id: "006_hide_resold_warranty_same_email_profile",
    name: "Hide resold warranty replacements by same email and profile",
    async up({ query }) {
        const [falsePositiveRows] = await query(`
        ...
        const falsePositiveIds = falsePositiveRows
            .map((row) => Number(row.id))
            .filter((id) => Number.isInteger(id) && id > 0);
        for (let i = 0; i < falsePositiveIds.length; i += 500) {
            const chunk = falsePositiveIds.slice(i, i + 500);
            const placeholders = chunk.map(() => "?").join(",");
            await query(
                `UPDATE subscriptions SET is_attended = 0 WHERE id IN (${placeholders})`,
                chunk
            );
        }
