            ...
            <form onSubmit={onSubmit}>
                ...
                <label style={S.label}>
                    ...
                    <div style={S.relativeWrap}>
                        <input style={{ ...S.input, width: "100%", paddingRight: 44 }} type={showPwd ? "text" : "password"} placeholder="Minimo 8 caracteres" value={password} onChange={(event) => setPassword(event.target.value)} onFocus={(event) => (event.target.style.boxShadow = "0 0 0 3px rgba(37,99,235,.25)")} onBlur={(event) => (event.target.style.boxShadow = "none")} required />
                ...
                <label style={S.label}>
                    ...
                    <div style={S.relativeWrap}>
                        <input style={{ ...S.input, width: "100%", paddingRight: 44 }} type={showConfirm ? "text" : "password"} placeholder="Repite tu contrasena" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} onFocus={(event) => (event.target.style.boxShadow = "0 0 0 3px rgba(37,99,235,.25)")} onBlur={(event) => (event.target.style.boxShadow = "none")} required />
                ...
                {isMobile ? (
                    <div style={{ marginTop: 20, textAlign: "center", fontSize: 13, color: C.muted }}>
                        Ya tienes cuenta?{" "}
                        <span onClick={toggle} style={{ color: "#0ea5e9", fontWeight: 700, cursor: "pointer" }}>Inicia Sesion</span>
                    </div>
                ) : null}
            </form>
        </div>
