...
const inputStyle = {
    appearance: "none", height: 36, padding: "0 12px 0 32px",
    background: "var(--bg0)", color: "var(--text)",
    border: "1px solid var(--stroke)", borderRadius: 10,
    fontSize: 13, fontWeight: 500, outline: "none", width: 220,
            ...
            {!loading && prices.length > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", borderTop: "1px solid var(--stroke)", background: "rgba(0,0,0,0.15)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--muted)", fontSize: 12 }}>
                        <span>Filas:</span>
                        <div style={{ position: "relative" }}>
                            <select style={selStyle} value={limit} onChange={e => loadAll(1, Number(e.target.value))}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <button className="btn-ghost" disabled={page <= 1 || loading} onClick={() => loadAll(page - 1, limit)}
                        ...
                        <button className="btn-ghost" disabled={page >= totalPages || loading} onClick={() => loadAll(page + 1, limit)}
